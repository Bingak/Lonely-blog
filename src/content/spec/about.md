...
- **墨驿 InkPost**：藏在站内的零依赖、离线可用静态后台. 文章配置全部表单化——标题/描述里的引号、冒号自动转义，不用手写一行 YAML；编辑内容自动保存、刷新不丢；内置多文档库、浅色/深色/跟随系统三态主题、本地 Markdown 导入和写作快捷键（Ctrl+B 加粗之类）. 写完一键提交到 GitHub 仓库，Cloudflare 自动构建上线喵~ 支持文章/项目/动态/相册四类，相册已迁移为内容集合，每个相册一个 `src/content/gallery/{id}.md` 文件，图片列表写在 frontmatter 的 `photos` 数组里. 还能从 GitHub 仓库拉取已有内容在线编辑、相册批量添加图片、视频嵌入自动识别 B 站/YouTube 且可设宽高、表单按分类折叠并标注必填/选填、编辑器与预览支持横向滚动、移动端也能正常使用喵~
- **登录安全**：用户名和密码存在 Cloudflare Worker 的 Secrets 里，前端拿不到明文；登录要过 Cloudflare Turnstile 人机验证，接口带同 IP 限流，成功后签发 12 小时的有效会话，锁定即失效. 入口藏在导航栏「链接」下拉菜单里，访客一眼看不出端倪.
### ⚙️ 个性化配置
