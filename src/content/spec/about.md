...
- **墨驿 InkPost**：藏在站内的零依赖、离线可用静态后台. 文章配置全部表单化——标题/描述里的引号、冒号自动转义，不用手写一行 YAML；编辑内容自动保存、刷新不丢；内置多文档库、浅色/深色/跟随系统三态主题、本地 Markdown 导入和写作快捷键（Ctrl+B 加粗之类）. 写完一键提交到 GitHub 仓库，Cloudflare 自动构建上线喵~ 支持文章/项目/动态/相册四类，相册会按专辑模型自动合并相册配置与图片列表.
