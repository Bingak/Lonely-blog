...
个人博客搭起来之后，最想知道的一件事往往是：**到底有没有人来？来了多少次？**

这篇教程用 **不蒜子（Busuanzi）** 这个极简网页计数器来回答这个问题。它的好处很适合个人博客：

- **零后端、零配置**：一个异步加载的 `<script>` 就搞定，不用自建数据库，也不用申请 API Key
- **一行标签一个数值**：想显示什么，就放对应的 `span` 占位
- **免费、轻量**：脚本只有几 KB，异步加载不影响页面速度

最终实现三个效果：**站点总访问量（PV）**、**访问人数（UV）**，以及**建站到现在运行了多久（年/天/时/分/秒）**。

> [!NOTE]
> 本教程以 **Jekyll / GitHub Pages** 博客为例（修改 `_includes/footer.html`）。如果你用的是 Hexo、VuePress、Hugo 等其它框架，思路完全一样——只要找到底部页脚文件，把下面两段代码贴进去即可。
...
刷新页面，稍等一两秒（首次统计可能需要等脚本回源），数字就出来了。

## 🚀 方法二：访问人数 + 总访问量 + 建站时间

如果想要更完整的效果（含 UV 和建站运行时间），把下面这段整体贴到 `_includes/footer.html` 的 `copyright` 附近：
...
> [!TIP]
> **JavaScript 的月份是从 0 开始数的**：`0 = 1 月`，`6 = 7 月`，`11 = 12 月`。上面那行 `6` 实际是 **7 月 24 日 20:00**。很多人照抄后发现自己博客「凭空多/少了几个月」，基本都是栽在这个 0-based 月份上。填之前先把月份减 1。

### 2. 文案可以随便改

代码里的 `🕓 Blog already run ...`、`👤Total Visitors`、`👁️Total Views` 都只是文字和 emoji，按你的博客风格改即可。emoji 可以在常用表情网站（如 emojixd）里挑。
...
> [!NOTE]
> 不蒜子官网：[https://busuanzi.ibruce.info/](https://busuanzi.ibruce.info/)。本文配置思路参考自 ThreeStones1029 的博客教程，并结合常见踩坑点做了补充。
