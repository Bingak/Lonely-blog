---
title: 今天更新了许多内容，大改了下博客
published: 2026-09-20T23:30:00+08:00
pinned: true
description: 2026-09-20 大改记录：导航栏聚光灯效果 + 胶囊圆角 + 个人资料卡、更新日志页（GitHub 提交集成 + 按天聚合 + 自动分类）、首页文章网格布局、背景壁纸全屏英雄模式，以及一堆小改动。35 次提交，从早上 8 点干到晚上 11 点。
...
先说结论：**35 次提交，从早上 8 点干到晚上 11 点**，改了导航栏、加了更新日志页、首页换网格布局、背景壁纸大改……基本把博客翻了个底朝天喵~

![今天的大改动概览](https://img.lonelybing.top/file/post/个人.png)
...
| 类型 | 匹配关键词 |
| --- | --- |
| 新功能 | `feat` `feature` `add` `新增` `添加` `实现` `完成` |
...
const KIND_PATTERNS: Array<{ kind: CommitKind; re: RegExp }> = [
  { kind: "feat", re: /^\s*(?:\[|\(|\{)?\s*(?:feat|feature|add|新增|添加|实现|完成)/i },
...
今天总共 **35 次提交**，时间线大概是这样：

| 时段 | 干了啥 |
| --- | --- |
| 08:00 - 12:00 | 小改动：动态更新、禁用看板娘、文章修正、图片清理 |
| 12:00 - 15:00 | 打赏者列表、.pages.yml、背景壁纸全屏 |
| 15:00 - 18:00 | 导航栏大改造（聚光灯 + 胶囊 + 滑动指示器） |
| 18:00 - 20:00 | 个人资料卡、页脚文案 |
| 20:00 - 23:00 | 更新日志功能 + 首页网格布局 + 卡片优化 |
